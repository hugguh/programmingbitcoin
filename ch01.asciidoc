...
[role="pagenumrestart"]
== Finite Fields/ Конечные поля

[.lead]


Одной из самых сложных вещей в изучении программирования Биткойн является понимание того, с чего начать.
Существует так много компонентов, которые зависят друг от друга, что изучение одного может привести к тому, что вам придется изучать другой, что, в свою очередь, может привести к необходимости изучения чего-то еще, прежде чем вы сможете понять то с чего вы начинали.

Эта ((("Bitcoin", "overview of components")))((("elliptic curve cryptography", "role in Bitcoin")))((("signing and verification", "purpose of")))((("transactions", "role in Bitcoin"))) глава поможет вам понять с чего начать.
Это может показаться странным, но мы начнем с базовой математики, которая необходима для понимания криптографии на эллиптических кривых.
Криптография на эллиптической кривой, в свою очередь, дает нам алгоритмы подписи и проверки.
Они лежат в основе работы транзакций, а транзакции являются атомарной единицей переноса стоимости в биткойнах.
Узнав о конечных полях ((("finite fields", "role in Bitcoin")))((("elliptic curves", "role in Bitcoin"))) и в первую очередь об эллиптических кривых, вы получите точное представление о концепциях, которые вам понадобятся, чтобы прогрессировать логически.

Имейте в виду, что эта и следующие две главы могут показаться немного похожими на то, что вы едите овощи, особенно если вы долгое время не занимались формальной математикой.
Я бы посоветовал вам пройти через них, поскольку представленные здесь концепции и код будут использоваться на протяжении всей книги.

=== Learning Higher-Level Math/ Изучаем высшую математику

Изучение новых математических структур может быть немного пугающим, я надеюсь что в этой главе сумею развеять миф о том, что высшая математика сложна.
Конечные поля, в частности, не требуют на много больше в плане глубоких математических знаний, чем, скажем, алгебра.

Думайте о конечных полях, как о том, что вы бы могли изучать в школе, вместо тригонометрии, но система образования, частью которой вы являетесь, решила, что изучение тригонометрии более важно для вас.
Таким образом я пытаюсь сказать вам, что конечные поля не так трудно изучить и для этого требуется не больше знаний, чем для изучения алгебры.

Эта глава необходима, если вы хотите понять криптографию на эллиптической кривой.
Криптография на эллиптической кривой необходима для понимания подписи и проверки, которые лежат в основе самого Bitcoin.

=== Finite Field Definition/ Определение Конечного поля
...
Математически,((("mathematical tools", "finite fields", id="MTfinite01")))((("finite fields", "definition of"))) _конечное поле_ определяется как конечный набор чисел и два оператора *+* (сложение) и *⋅* (умножение), которые удовлетворяют следующим требованиям:


1. Если _a_ и _b_ включены в конечный набор чисел, __a + b__ и __a__ ⋅ __b__ также входят в этот ряд чисел. Это свойство называется _закрытым_.
2. 0 exists and has the property __a__ + 0 = __a__. We call this the _additive identity_.
2. 0 существует и __a__ + 0 = __a__. Это свойство называется
